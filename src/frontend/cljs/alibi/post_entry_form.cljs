...
(defn active-errors
  [for-state]
  (let [{:keys [input-entry]} for-state
        {:keys [formWasSubmitted]} input-entry]
...
(defn summary-errors
  [for-state]
  (let [{:keys [input-entry]} for-state
        {:keys [formWasSubmitted submit-time-state]} input-entry]
    (seq (when formWasSubmitted
...
(defn validation-summary
  [for-state]
  (let [summary-errors (summary-errors for-state)]
    (dom/div
      #js {:className
           (str "alert entry-form-errors"
                (if (active-errors for-state) " alert-danger" " alert-success")
...
(defn date-time-entry-row [for-state]
  (let [{:keys [input-entry
        ...
        failed-fields (into {} (active-errors for-state))
...
(defn comment-row [for-state]
  (let [{:keys [input-entry
                dispatch!]} for-state
        {:keys [comment]} input-entry]
...
(defn billable?-row [for-state]
  (let [{:keys [input-entry
                dispatch!]} for-state
        {:keys [isBillable]} input-entry]
    (dom/div
      #js {:className "form-group"}
      (dom/div
...
(defn render-state
  [for-state]
  (let [{:keys [input-entry dispatch!]} for-state
      ...
      (dom/form
        ...
        (dom/div
          #js {:className (string/join
                            " "
                            [(when (summary-errors for-state) "has-errors")
            ...
            (dom/div
              ...
              (dom/div
                ...
                (dom/div
                  ...
                  (dom/fieldset
                    nil
                    (date-time-entry-row for-state)
                    (comment-row for-state)
                    (billable?-row for-state)
                    (btn-row for-state)))))))))))
